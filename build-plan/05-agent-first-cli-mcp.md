...
Claude Code's current CIMD declares the portless `http://localhost/callback` URI but its CLI normally chooses an available port and requests `http://localhost:<port>/callback`. The compatibility layer may add only that exact port-bearing URI to Claude's freshly fetched metadata for the current authorization transaction. It must first verify the exact Claude metadata client ID, authorization endpoint, one occurrence each of client and redirect, `http` scheme, `localhost` host, numeric port `1-65535`, `/callback` path, absence of credentials/query/fragment, and the metadata document's original portless callback. Because CIMD is resolved again during consent and post-login continuation, those bounded POST phases must recover and revalidate the same callback from the provider-signed `oauth_query`; they must not trust `Referer`, a browser-added header, an unsigned field, or a cached port. This does not create a wildcard registration or weaken PKCE, state, resource, scope, consent, expiry, or audience validation. RFC 8252's loopback guidance explains the ephemeral-port requirement and recommends IP literals over `localhost`; the exception remains pinned to Claude until the client changes.
