...
16. **The API timed out a healthy runner long poll before its advertised bound.** The runner requests `runner_wait` for at most 20 seconds, but both the outer API gateway and its API-to-Convex executor originally kept shorter generic timeouts. Extending only the inner executor left the outer gateway free to abort first, changing the live symptom from `temporarily_unavailable` to `request_cancelled`. Every bounded long-poll operation must carry its requested wait into every enclosing and upstream timeout budget, with a small completion margin, and regressions must exercise the maximum advertised wait at each layer.
