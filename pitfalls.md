...
16. **The API timed out a healthy runner long poll before its advertised bound.** The runner requests `runner_wait` for at most 20 seconds, but the API-to-Convex executor extended its 15-second internal timeout only for `get_updates`. With no new job to refill, a valid dispatcher wait was aborted at 15 seconds and surfaced as repeated `temporarily_unavailable`; one live wiwi runner reached 56 consecutive failures while its exact job process remained healthy. Every bounded long-poll operation must carry its requested wait into each upstream timeout budget, with a small completion margin, and a regression must exercise the maximum advertised wait.
17. **macOS process discovery can disclose a secret-bearing child's full invocation.** `pgrep -fl` and `ps` modes that render full command strings can include environment assignments or arguments supplied by a release harness. During any credential-bearing run, inspect only a fixed safe field list such as PID, PPID, process group, state, elapsed time, and executable name; never capture full argv or environment. If a diagnostic trace does capture a credential, stop at the next safe release boundary, rotate it through the documented owner path, and verify the replacement without printing its value.
