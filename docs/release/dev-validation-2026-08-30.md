...
| Gate | Evidence |
|---|---|
| Local unit/integration suite | `npm test`: 221 tests passed across Workers, CLI, local storage, web, MCP, contracts, and Convex before the subsequent focused changes; each subsequent changed package also passed its focused suite. A final full rerun remains required. |
...
| Static/type/build checks | `npm run check` passed. Contract generation, lowercase brand verification, environment boundaries, secret scan, and safe-runtime-log checks passed. |
...
| Packaged CLI in this repository | Packed `@dongo/cli@0.1.0`, installed the tarball into a clean temporary prefix, and ran that exact binary from this repository. `doctor`, `session-start`, `overview`, and deterministic `sync` succeeded against the live project. A second `sync` made no changes. |
...
| OAuth callback safety | MCP consent uses a top-level host-owned loopback redirect. dongo does not frame, fetch, or proxy localhost, eliminating the browser private-network/device-access prompt. |
...
- Rerun the complete suite after all final changes, then repeat secret, runtime-log, contract, environment, browser, and live smoke gates on the exact candidate commit.
