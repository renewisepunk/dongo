...
| Fresh-account agent-first preflight | A new disposable `test@paul9.com` identity completed email OTP and landed on first-project onboarding with no existing project. The packed CLI then opened one complete device link proposing `dongo e2e`, `https://github.com/renewisepunk/dongo`, and manual execution. The browser showed that fixed proposal with the same comparison code as the waiting terminal and no project picker. Project creation and grant issuance remain pending explicit action-time approval. |
...
- With user confirmation, click the pending `Create & approve` action for the clean `test@paul9.com` identity, then verify packaged-CLI storage, doctor, first-project metadata, and logout/revocation behavior.
