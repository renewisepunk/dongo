...
The current dev-only goal is **not yet complete**. The uninterrupted CLI/human golden journey is now proven. Its remaining evidence includes equivalent complete Codex/Claude/generic MCP journeys, live Google identity proof, a real browser-to-R2 media journey, and clean-host provenance. Native clients, push, staging, rollback rehearsal, and a production application candidate remain outstanding against the full PRD but are outside the current build goal.
...
| One-link CLI authentication | Proven in development | The packed CLI completed terminal → browser → authenticated terminal with no token copy, project picker, Keychain prompt, or repository credential. A separate disposable grant then proved successful server revocation, local credential removal, and isolation from the retained repository grant. | Prove deny/expiry/headless on clean macOS and Linux hosts. |
...
| Packed CLI lifecycle | Proven in development; clean Linux install proven | The exact packed archive created, started, updated, and finished `DONGO-4`; every mutation replay returned the original result. A second deterministic sync returned identical hashes. A separate disposable grant proved logout/revoke isolation, and a final isolated CLI run completed live Intake triage, important human Attention, next-pull continuation, finish, artifact, and deterministic export as `DONGOE2E-1`. The same immutable archive (SHA-256 `e04fcc2fcc10e552417c046450531936985ff89b5b03504bcd2848f4add30990`) also installed and returned a truthful unauthenticated status in a fresh Debian/Node 20 container with no workspace source or credential helper. | Complete browser-approved `--no-browser` auth and the lifecycle on clean Linux, then repeat the clean-host proof on macOS. |
| Web product behavior | Proven in automated browser matrix; live high-zoom proven | 216 Playwright tests pass across Chromium, Firefox, and WebKit, including Overview, detail, responsive states, paste image, full-page drop, upload retry/cancel, search, settings, help, keyboard shortcuts, capture-boundary navigation, prominent copyable issue identifiers, and auth/consent states. On the authenticated development candidate, native Chrome 200% zoom preserved named controls, trapped detail focus, and introduced no horizontal page overflow at the resulting 900 CSS-pixel viewport; Chrome was restored to 100%. | Perform the live media and golden human/agent journeys, plus manual screen-reader and measured performance review on the release candidate. |
...
| Operational release | Partially in current goal | Development runbooks, CI, safe logs, health/readiness, and per-service observability configuration exist. Clean Debian/Node 20 now proves exact-package installation, isolated unauthenticated status, owner-only empty configuration, no repository marker, and no OS credential helper. | For the dev-only goal, complete the clean-host OAuth/lifecycle matrix and the relevant development observability checks. Staging rollback, production publication, and production support sign-off are deferred. |
| Exact PRD V1 success journey | Partial as one uninterrupted candidate run | One live CLI/human run completed text Intake → triage → claim → truthful Working → important human review/response → next pull → finish → artifact/outcome → deterministic repository Markdown on the same deployed candidate. | Add real image/video media to a deployed journey and repeat the complete semantics through Codex, Claude Code, and a generic MCP client. |

## Required execution order

1. Finish the live Google identity proof; disposable CLI logout/revocation is proven.
2. Repeat the now-proven complete CLI/human journey through Codex, Claude Code, and a generic MCP client, including lost-response replay and Attention response.
