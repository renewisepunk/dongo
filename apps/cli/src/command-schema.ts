...
export const COMMAND_SCHEMAS: Record<string, CommandSchema> = {
  ...
  "work update": {
    ...
    options: [
      ...
      { name: "latest-update", description: "Meaningful Run progress or blocker. Concise Markdown is supported." },
  ...
  "comment add": { command: "comment add", summary: "Add a Markdown comment to a WorkItem.", usage: "dongo comment add --work-id ID --body TEXT", options: [workId, { name: "body", description: "Comment body. Concise Markdown is supported.", required: true }, idempotency] },
  "attention request": {
    ...
    options: [
      ...
      { name: "body", description: "Request details. Concise Markdown is supported.", required: true },
